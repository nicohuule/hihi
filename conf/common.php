...
function ApiKeyValid($apiKey) {
    $apiurl = 'https://thanhdieu.com/api/check-liscense.php?check=' . urlencode($apiKey);
    $urlParts = parse_url($apiurl);
    $host = $urlParts['host'];
    if ($host !== 'thanhdieu.com') {
        die('Lỗi source code, liên hệ cho chủ sở hữu mã nguồn ngay!');
    }
    ...
    if ($response === false) {
        die('Không thể kết nối đến thanhdieu.com');
    }
    if (strpos($response, 'captcha') !== false) {
        die('Phát hiện tường lửa, không thể kết nối.');
    }
    ...
    if ($httpCode === 200 && !empty($data)) {
        ...
        die('ThanhDieuLiscense từ chối kết nối, hoặc gặp lỗi nghiêm trọng, vui lòng liên hệ cho chủ sở hữu code này và gửi mã lỗi: `Server refused connection`');
    }
    return false;
}

$apiResult = ApiKeyValid(API_KEY);
if ($apiResult && isset($apiResult['results'][API_KEY])) {
    $status = $apiResult['results'][API_KEY]['status'];
    $reason = isset($apiResult['results'][API_KEY]['reason']) ? $apiResult['results'][API_KEY]['reason'] : '';
    if ($status === 'on') {
